...
@pytest.fixture
def script():
    return "./src/monarch_ingest/ingests/alliance/gene_to_phenotype.py"
...
def test_association_publication(rat):
    association = [association for association in rat if isinstance(association, GeneToPhenotypeAssociation)][0]
...
def test_conditions(conditions_entities):
    association = [
        ...
        if isinstance(association, GeneToPhenotypeAssociation)
    ][0]
