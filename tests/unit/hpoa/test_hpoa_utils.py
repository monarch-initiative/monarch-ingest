...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("HP:0040279", None),  # query 3 - the sub-ontology term below HP:0040279, not this 'Frequency' term itself, LOL
        (  # query 4 - exact matches to global lower bounds should be sent back accurately
            "0%",
            FrequencyHpoTerm("HP:0040285", "Excluded", 0, 0),
            0.0,
            None,
        ),
        (  # query 5 -exact matches to lower bounds should be sent back accurately
            "5%",
            FrequencyHpoTerm("HP:0040283", "Occasional", 5, 29),
            5.0,
            None,
        ),
        ...
        (  # query 8 - exact matches to global upper bounds should be sent back accurately
            "100%",
            FrequencyHpoTerm("HP:0040280", "Obligate", 100, 100),
            100.0,
            None,
        ),
        (  # query 9 - if a valid 'Frequency' HPO subontology term  already, should be sent back
            "HP:0040282",
            FrequencyHpoTerm("HP:0040282", "Frequent", 30, 79),
            None,
            None,
        ),
        ...
        ("1/2", FrequencyHpoTerm("HP:0040282", "Frequent", 30, 79), None, 0.5),  # query 13 - frequent percentage value
    ],
)
def test_phenotype_frequency_to_hpo_term(query: Tuple[str, Optional[FrequencyHpoTerm]]):
    ...
    result: Optional[Tuple[FrequencyHpoTerm, Optional[float], Optional[float]]] = phenotype_frequency_to_hpo_term(
        query[0]
    )
    if query[1]:
