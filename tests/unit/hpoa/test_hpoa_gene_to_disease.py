...
from monarch_ingest.constants import INFORES_MONARCHINITIATIVE, INFORES_OMIM, INFORES_ORPHANET, INFORES_MEDGEN, BIOLINK_CAUSES, BIOLINK_CONTRIBUTES_TO, BIOLINK_GENE_ASSOCIATED_WITH_CONDITION
...
@pytest.mark.parametrize(
     ("original_source", "expected_primary_knowledge_source", "expected_aggregator_knowledge_source"),
         [
             ("ftp://ftp.ncbi.nlm.nih.gov/gene/DATA/mim2gene_medgen", INFORES_OMIM, [INFORES_MEDGEN, INFORES_MONARCHINITIATIVE]),
             ("http://www.orphadata.org/data/xml/en_product6.xml", INFORES_ORPHANET, [INFORES_MONARCHINITIATIVE])
         ]
     )
def test_knowledge_source(original_source: str,
                          expected_primary_knowledge_source: str,
                          expected_aggregator_knowledge_source: List[str]):
    primary_knowledge_source, aggregator_knowledge_source = get_knowledge_sources(original_source, INFORES_MONARCHINITIATIVE)
...
@pytest.mark.parametrize(("association", "expected_predicate"),
        [
...
@pytest.fixture
def row():
    return {'association_type': 'MENDELIAN',
